...
4.7.  Section 1.4 of the SAM/BAM documentation indicates V5 as the mapping quality. The number of reading iwth mapping quality less than 50 is:

```{r}
sum(sam$v5 < 50)
```

4.8. The mean mapping quality of the reads which have mapping quality less than 50 is:
```{r}
mean(sam$v5[sam$v5 < 50])
```

4.9. The number of reads which align to the tdTomato sequence is:
...
Coding variant will have specific keywords, we want only those variants. Then we can filter out the HIGH impact coding variant
